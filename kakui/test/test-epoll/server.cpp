...
void handleAccept(int efd, int fd)
{
	...
	setNonBlock(cfd);
	updateEvent(efd, cfd, EPOLLIN, EPOLL_CTL_ADD);
}
...
void sendRes(int efd, int fd)
{
	...
	if(!con.readed.length())
	{
		if(con.writeEnabled)
		{
			updateEvent(efd, fd, EPOLLIN, EPOLL_CTL_MOD);
	...
	if(wd < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
	{
		if(!con.writeEnabled)
		{
			updateEvent(efd, fd, EPOLLIN|EPOLLOUT, EPOLL_CTL_MOD);
...
void loopOnce(int efd, int lfd, int waitms)
{
...
int main(int argc, char* argv[])
{
	...
	exit_if(epollfd < 0, "epoll_creat() failed");
	...
	updateEvent(epollfd, listenfd, EPOLLIN, EPOLL_CTL_ADD);
	for(;;)
	{
