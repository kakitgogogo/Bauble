...
def cpu_info():
	...
	else_percent = 0.0
	for i in range(5, 10):
		else_percent += cpu_times_percent[i]
	...
	if part == 'disk':
		...
		for physical_disk_partition in physical_disk_partitions:
			...
			physical_disk = {
				...
				'mountpoint': physical_disk_partition.mountpoint,
...
def all_processes(sort='cpu', order='asc'):
	processes = []
	for proc in psutil.process_iter():
		...
		processes.sort(key=lambda p: p.get(sort), reverse=True if order=='desc' else False)
